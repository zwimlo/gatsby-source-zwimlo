...
export default async ({
  ...
  const apiEndpoint = `https://${apiType}.zwimlo.com/v1/sites/${siteId}/${pluralize(
    objectType,
  )}`
...
const pagedGet = async (
  ...
  {
    verbose &&
      console.log(`${chalk.magentaBright('zwimlo')} current offset: ${offset}`)
  }
  {
  ...
  console.log(
    `${chalk.black.bgCyan(' zwimlo ')} fetched successfully ${chalk.cyan(
      aggregatedResponse.length,
    )} items`,
  )
